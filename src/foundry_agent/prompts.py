...
VALIDATION_INSTRUCTIONS = (
    ...
    "1. DETERMINISTIC PRESENCE CHECK — ALWAYS call the run_skill_validation tool before "
    "judging anything; never skip it or guess its result. Extract the current value of "
    "every attribute the candidate content populates, keyed by its exact attribute id "
    "(e.g. {'PA1': 'POL-SEC-001', 'PA2': 'Remote Work Security Policy'}), and pass that "
    "mapping to the tool. It runs the format skill's own deterministic rules — required-"
    "field presence, placeholders, format rules such as a title word cap — and returns "
    "the attribute ids that fail them.\n"
