...
async def test_an_oversized_initial_paste_is_capped_before_it_enters_the_run(
    ...
    clients: dict = {}
    workflow = _workflow(make_stub_client, make_elicitation_client, clients=clients)
    huge = "PASTE " * (MAX_USER_INPUT_CHARS // 3)  # well over the cap

    await workflow.run(huge)

    # The first stage to read run.content is the elicitation open (no analysis stage).
    first_prompt = clients["elicit"].prompts[0]
    assert "[input truncated:" in first_prompt
    assert len(first_prompt) < len(huge)
